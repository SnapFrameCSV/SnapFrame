...
- Export PNG at 1× and 2×: build an SVG from the highlighted HTML (SVG is the source of truth) and rasterise via `Image` → `canvas` → `toBlob`. **Not** with a `<foreignObject>` around the HTML (the original plan): browsers unconditionally taint any canvas an SVG image containing a foreignObject is drawn into, so `toBlob` throws — see D17. Instead the webview walks the captured DOM into per-line styled text runs and the SVG draws them as native `<text>`/`<tspan>`. Save via `showSaveDialog` (default name `snapframe-<file>-<line>.png`). Copy-to-clipboard as an *image* via `navigator.clipboard.write([new ClipboardItem({'image/png': blob})])` inside the webview; if the platform refuses, fall back to saving the file and say so plainly in the status bar — never silently copy text.
